...
# -*- coding: utf-8 -*-
"""
Build a fully decoded CSV from the official Légis Québec page (S-2.1, r. 4),
with EPUB fallback and a local test mode that can run without internet.

Features
--------
- Downloads HTML (curl -> wget -> requests)
- Extracts only <div id="mainContent-document">
- Parses ALL ids (se:/ss:/p1:/p2:/p3:/df:, ga:, sc-nb:, others)
- Splits historical notes (R.R.Q., D., L.Q.) from MainText
- Decodes IDs to human refs with strict spacing rules:
  * Article underscores -> dots (2_1_1 -> 2.1.1)
  * p1 numeric -> "al. N" (spaces around 'al.')
  * p2 numeric -> "(N)" with spacing logic:
      - if we already had "al. N": add a space before "(N)" => "al. 2 (3)"
      - if NO p1 at all: attach directly to article => "29(3)"
      - if p1 letter/roman: add a space before "(N)"
  * letters -> "a)", "b)"
  * romans -> "i)", "ii)" appended directly (e.g. "...a)ii")
- EPUB fallback:
  * Finds .epub link on the HTML page (if available) OR use --epub-url
  * Parses all OPF/*.xhtml pages and applies same logic
- Local test mode (--test-local) that uses an embedded HTML fixture so you can
  validate decoding in offline environments (like this canvas).

Output CSV columns
------------------
ID, Type, Article, MainText, HistoricalNote, DecodedRef, DecodedParts, DecodedTerm

Usage
-----
python build_codesecurite_csv.py --url 'https://www.legisquebec.gouv.qc.ca/fr/document/rc/S-2.1,%20r.%204' --out codesecurite_with_decoded.csv
python build_codesecurite_csv.py --from-epub --url '...' --out codesecurite_with_decoded.csv
python build_codesecurite_csv.py --epub-url 'https://.../S-2.1,%20r.%204.epub' --out codesecurite_with_decoded.csv
python build_codesecurite_csv.py --test-local --out test.csv
"""
...
# EPUB parsing
# -------------------------
def iterate_ids_from_epub(epub_path: str) -> Iterable[Tuple[str, str]]:
    import zipfile
    with zipfile.ZipFile(epub_path, "r") as z:
        names = [n for n in z.namelist() if n.lower().endswith(".xhtml")]
        def page_order(n: str) -> Tuple[int, str]:
            import re as _re
            m = _re.search(r"page(\d+)\.xhtml$", n)
...
def run_from_html(url: str, save_html: str) -> List[Dict[str, str]]:
    ...
    html_bytes = open(save_html, "rb").read()
...
def main():
    ap = argparse.ArgumentParser(description="Build codesecurite_with_decoded.csv from Légis Québec (HTML with EPUB fallback).")
    ap.add_argument("--url", "-u", default="https://www.legisquebec.gouv.qc.ca/fr/document/rc/S-2.1,%20r.%204",
                    help="Légis Québec URL for S-2.1, r. 4")
    ap.add_argument("--out", "-o", default="codesecurite_with_decoded.csv",
