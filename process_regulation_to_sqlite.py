...
storing the results in a SQLite database.
"""
...
def setup_database_table(conn):
    ...
    CREATE TABLE IF NOT EXISTS {table_name} (
        ...
        SourceURL TEXT,
        AbrogationNotice TEXT,
        OfficialStatus TEXT,
        PdfURL TEXT,
        PRIMARY KEY (ID, VersionDate)
    )""")
    conn.commit()
...
def insert_data_to_sql(conn, rows: List[Dict[str, str]]):
    ...
    insert_count = 0
    for row in rows:
        # Using INSERT OR IGNORE to handle duplicates based on the PRIMARY KEY
        columns = ', '.join(row.keys())
        placeholders = ', '.join(['?' for _ in row])
        sql = f"INSERT OR IGNORE INTO {table_name} ({columns}) VALUES ({placeholders})"
        result = cursor.execute(sql, list(row.values()))
...
def download_file(url: str, out_path: str, accept: Optional[str] = None) -> None:
    ...
    if wget:
        ...
        subprocess.run(cmd, check=True, capture_output=True)
        if os.path.exists(out_path) and os.path.getsize(out_path) > 0: return
    if 'requests' not in sys.modules: raise RuntimeError("'requests' not available.")
...
def main():
    ap = argparse.ArgumentParser(description="Archive all historical versions of a Légis Québec regulation to a SQLite database.")
    ...
    ap.add_argument("--db-path", default="legislation.db", help="Path to the SQLite database file")
    ...
    conn.close()
    print(f"\n[SUCCESS] Database archival complete. Data stored in {os.path.abspath(args.db_path)}")
